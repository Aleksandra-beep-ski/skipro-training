...
function processBooking(trainingType) {
    try {
        ...
        // Отправляем уведомления (только одну функцию!)
        sendBookingEmailNotification(bookingData);
        
        console.log('Уведомления отправлены');
        
        // НЕ закрываем модальное окно здесь - оно закроется при отправке письма
        ...
        setTimeout(() => {
            ...
            if (typeof renderBookedTrainings === 'function') {
        ...
        showNotification('Произошла ошибка при бронировании. Попробуйте еще раз.', 'error');
    }
}

/**
 * Отправка уведомления о бронировании тренировки (объединенная функция)
...
function sendBookingEmailNotification(bookingData) {
    ...
    const adminSubject = `Новая бронь тренировки: ${bookingData.trainingType}`;
    const adminBody = `
...
Это сообщение отправлено автоматически с сайта SkiPro Training.
    ...
    const userSubject = `Подтверждение бронирования тренировки: ${bookingData.trainingType}`;
    const userBody = `
Уважаемый(ая) ${currentUser.name}!
...
${new Date().toLocaleDateString('ru-RU')}
    ...
    // Показываем модальное окно с письмом администратору
    sendEmail(ADMIN_EMAIL, adminSubject, adminBody);
    
    // Также отправляем подтверждение клиенту (без показа модального окна)
    setTimeout(() => {
        sendEmail(currentUser.email, userSubject, userBody, false);
    }, 1000);
}
...
/**
 * Обновленная функция обработки бронирования с проверкой доступности
 */
function processBooking(trainingType) {
    try {
        // Собираем данные бронирования
...
// Обновленная функция отображения модального окна бронирования
function showBookingModal(trainingType) {
    // Проверяем авторизацию
    if (!currentUser) {
        showNotification('Для бронирования необходимо войти в систему', 'error');
    ...
    function checkAvailability() {
        ...
        if (date && timeSelect.value) {
            if (isTimeSlotAvailable(date, timeSelect.value)) {
    ...
    // Навешиваем обработчики для проверки доступности
...
/**
 * Универсальная функция отправки email с возможностью скрытия модального окна
 * @param {string} to - Email получателя
 * @param {string} subject - Тема письма
 * @param {string} body - Текст письма
 * @param {boolean} showModal - Показывать ли модальное окно (по умолчанию true)
 */
function sendEmail(to, subject, body, showModal = true) {
    ...
    if (!showModal) {
        // Просто логируем отправку без показа модального окна
        return true;
    }
...
// Проверяем здоровье сервиса при загрузке
document.addEventListener('DOMContentLoaded', function() {
    checkServiceHealth().then(health => {
        if (health) {
            console.log('✅ Сервис работает нормально');
        } else {
            console.log('⚠️ Сервис недоступен, используется локальное хранилище');
        }
    });
...
function initFormHandlers() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            // не показываем уведомление здесь - оно покажется после отправки письма
            this.reset();
        });
    }
}

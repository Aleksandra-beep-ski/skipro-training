...
function processBooking(trainingType) {
    ...
    closeModal();
    showNotification('Тренировка успешно забронирована!', 'success');
...
function sendEmail(to, subject, body) {
    // В реальном проекте здесь будет код для отправки email через сервер
    // Сейчас используем EmailJS или аналогичный сервис
    ...
    // Альтернативный способ: открытие почтового клиента
    const mailtoLink = `mailto:${to}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    
    // Создаем временную ссылку для отправки
    ...
    showNotification('Уведомление отправлено администратору', 'success');
}

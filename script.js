...
/**
 * Универсальная функция отправки email (демо-версия)
    ...
    const modal = createModal(`
        ...
        <div class="email-demo">
            ...
            <div class="email-actions">
                <button onclick="sendRealEmail('${to}', '${subject}', \`${body.replace(/'/g, "\\'")}\`)" class="btn email-send-btn">
                    Открыть в почтовом клиенте
                </button>
                <button onclick="closeModal()" class="btn email-cancel-btn">
                    Только посмотреть
                </button>
            </div>
...
function sendRealEmail(to, subject, body) {
    ...
    setTimeout(() => {
        closeModal();
        showNotification('✓ Письмо открыто в почтовом клиенте! Нажмите "Отправить" для отправки.', 'success');
    }, 1000);
}
